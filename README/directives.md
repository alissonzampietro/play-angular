...
## Built-in directives
 ...
 - ```*ngIf```: conditional directive, that only shows the element where it is inserted, if the condition is *true*. **star is use to tell to interpreter that it's a structure directive that changes the DOM, it's called [Structural directives](https://angular.io/guide/structural-directives)**. 
 ...
 - ```*ngFor```: Allows you to loop over a list of elements, as you can [see here](../src/app/servers/servers.component.html). **star is use to tell to interpreter that it's a structure directive that changes the DOM**.