...
## Built-in directives
 ...
 - ```*ngIf```: conditional directive, that only shows the element where it is inserted, if the condition is *true*. **star is the syntax sugar to [Property Binding]**. 
 ...
 - ```*ngFor```: Allows you to loop over a list of elements, as you can [see here](../src/app/servers/servers.component.html). The asterisk is the syntax sugar to [Property binding](./dataBinding.md).